...
@pytest.mark.parametrize("a,b,expected", [
    (1e308, 1e308, 2e308),       # Very large numbers (addition)
    (-1e308, -1e308, -2e308),    # Very large negatives
    (1e-308, 1e-308, 2e-308),    # Very small numbers
])
def test_addition_extremes(a, b, expected):
    result = Operation.addition(a, b)
    assert math.isclose(result, expected, rel_tol=1e-12)

@pytest.mark.parametrize("a,b,expected", [
    (1e308, 1e308, 0.0),          # subtraction leading to 0?
    (-1e308, 1e308, -2e308),
    (1e-308, 0.0, 1e-308),
])
def test_subtraction_extremes(a, b, expected):
    result = Operation.subtraction(a, b)
    assert math.isclose(result, expected, rel_tol=1e-12)

@pytest.mark.parametrize("a,b,expected", [
    (1e154, 1e154, 1e308),       # large number multiplication
    (-1e154, 1e154, -1e308),
    (1e-154, 1e-154, 1e-308),
])
def test_multiplication_extremes(a, b, expected):
    result = Operation.multiplication(a, b)
    assert math.isclose(result, expected, rel_tol=1e-12)

@pytest.mark.parametrize("a,b,expected", [
    (1e308, 1e-308, 1e616),       # division overflow
    (-1e308, 1e-308, -1e616),
    (1e-308, 1e308, 1e-616),      # division underflow
])
def test_division_extremes(a, b, expected):
    # Division by extremely small numbers may result in OverflowError
    try:
        result = Operation.division(a, b)
        assert math.isclose(result, expected, rel_tol=1e-12)
    except OverflowError:
        pass  # acceptable if numbers exceed floating point limits
